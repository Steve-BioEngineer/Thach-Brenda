(function () {
  ...
  /* 3. (Optional) WebP fallback BEFORE Swiper starts */
const webpOK = (()=>{try{
  return document.createElement('canvas').toDataURL('image/webp').indexOf('data:image/webp')===0;
}catch{return false}})();
if(!webpOK){
  document.querySelectorAll('.slide-bg.swiper-lazy').forEach(el=>{
    const jpg = el.dataset.backgroundFallback;
    if(jpg) el.dataset.background = jpg;
  });
}
  ...
  const storySwiper = new Swiper('.story-swiper', {
    loop           : true,
    effect         : 'fade',
    preloadImages  : false,        // turn off eager loading
    lazy           : {
      loadPrevNext       : true,   // load active + adjacent slides
  ...
  if (!webpSupported) {
    document.querySelectorAll('.slide-bg.swiper-lazy').forEach((el) => {
      const jpg = el.dataset.backgroundFallback;
      if (jpg) el.dataset.background = jpg;     // swap to JPEG before Swiper lazy-loads
    });
  }
        /* --------------------------------------------------
     * GLightbox & Isotope
    ...
    const storyBox   = document.getElementById('loveStory');
    const photoBox   = document.getElementById('photoContainer'); // if you’re collapsing it

    btn.addEventListener('click', ()=>{

      const nowOpen = storyBox.classList.toggle('expanded');
      photoBox?.classList.toggle('collapsed', !nowOpen);

      btn.textContent = nowOpen ? 'Hide our story' : 'Read our story';
    });
